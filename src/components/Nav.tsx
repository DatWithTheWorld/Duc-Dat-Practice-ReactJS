...
const Nav: React.FC<NavProps> = ({style, theme, logo}) => {
    const BtnTheme = theme === "white" ? 'bg-white text-black' : 'bg-buttonDlBlack text-white';
    return (
        <nav className={`p-4 flex  justify-between items-center  rounded-2xl  ${style}`}>
            <a href="" className=" flex items-center">
                ...
                <p className="max-sm:hidden text-0.875rem font-bold text-nowrap" style={{color: theme === 'white' ? 'white' : 'black'}}>PURITY UI DASHBOARD</p>
            </a>
             <ul className="flex max-lg:hidden w-[45%] justify-between items-center">
                {
                    ...
                    <li className="flex gap-1.5 items-center justify-center uppercase font-bold text-0.625rem" key={item.id}>
                        ...
                        <a href="" className="text-nowrap" style={{color: theme === 'white' ? 'white' : 'black'}}>{item.title}</a>
                    </li>
                    ))
                }
            </ul>
            <ButtonDL style={BtnTheme}/>
        </nav>
    );
};
